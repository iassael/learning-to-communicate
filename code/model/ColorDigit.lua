...
return function(opt)
    ...
    function exp.save(opt, stats, model)
        if stats.e % opt.step == 0 then
            ...
            -- keep stats
            stats.history = stats.history or { { 'e', 'td_err', 'td_comm', 'train_r', 'test_r', 'steps', 'comm_per', 'te' } }
    ...
    local function create_agent()
        ...
        if (opt.game_comm_bits > 0) and (opt.game_nagents > 1) then
            comm_size = opt.game_comm_bits * opt.game_nagents
        end
        ...
        -- Networks
        local model_mnist = nn.Sequential()

        if opt.game_use_digits == 1 then
            model_mnist:add(nn.LookupTable(10, opt.model_rnn_size))
        else
            if opt.game_vision_net == 'mlp' then
                model_mnist:add(nn.View(-1, opt.game_colors * 28 * 28))
                if opt.model_bn == 1 then model_mnist:add(nn.BatchNormalization(opt.game_colors * 28 * 28)) end
                model_mnist:add(nn.Linear(opt.game_colors * 28 * 28, 128)) -- fully connected layer (matrix multiplication between input and weights)
                model_mnist:add(nn.ReLU(true))
                if opt.model_bn == 1 then model_mnist:add(nn.BatchNormalization(128)) end
                model_mnist:add(nn.Linear(128, opt.model_rnn_size))
                model_mnist:add(nn.View(-1, opt.model_rnn_size))
            else
                model_mnist:add(nn.SpatialConvolutionMM(opt.game_colors, 32, 5, 5)) -- 1 input image channel, 6 output channels, 5x5 convolution kernel
                model_mnist:add(nn.ReLU(true))
                model_mnist:add(nn.SpatialMaxPooling(3, 3, 3, 3)) -- A max-pooling operation that looks at 2x2 windows and finds the max.
                model_mnist:add(nn.SpatialConvolutionMM(32, 64, 5, 5))
                model_mnist:add(nn.ReLU(true))
                model_mnist:add(nn.SpatialMaxPooling(2, 2, 2, 2))

                local conv_out = model_mnist:forward(torch.zeros(1, opt.game_colors, 28, 28)):nElement()

                model_mnist:add(nn.View(-1, conv_out))
                model_mnist:add(nn.Linear(conv_out, opt.model_rnn_size))
                model_mnist:add(nn.View(-1, opt.model_rnn_size))
            end
        end
